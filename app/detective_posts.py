...
def media(day, slot):
    period = 'noon' if slot['start'].hour == 12 else 'night'
    relative = f'static/detective/{period}.jpg'
    return relative if (ROOT / relative).is_file() else None


def run(db, sender, pause, enabled, configured, init_quote, cap, cooldown):
    ...
    slot = plan.current_slot(now, 'character')
    if not slot:
    ...
    with db() as c:
        ...
        reserved, reason = plan.reserve(c, 'character', now, cap, cooldown)
        if not reserved:
            return {'sent':0, 'reason':reason}
        response = sender(plan.character_text(day, slot), 'https://buzz-now-1.onrender.com/'+asset, 'shareNow')
        if response.get('ok'):
