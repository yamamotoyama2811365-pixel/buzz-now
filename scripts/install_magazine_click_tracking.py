...
Stores only JST day + source path + target path + placement + test flag + click count.
No IP, cookie, session id, user agent, referrer URL, query string, or persistent nonce.
"""
...
def apply_python():
    ...
    if 'visitor_internal_clicks_daily' in text:
        return
    anchor = 'STATE_SCHEMA = """CREATE TABLE IF NOT EXISTS visitor_analytics_state ('
    pos = text.index(anchor)
    end = text.index('\n"""\n', pos) + len('\n"""\n')
    text = text[:end] + CLICK_SCHEMA + text[end:]
    init_anchor = '        connection.execute(STATE_SCHEMA)\n'
    text = text.replace(init_anchor, init_anchor + '        connection.execute(CLICK_SCHEMA)\n', 1)
    rec_anchor = '\ndef install_visitor_analytics(app, db, is_legacy=False):\n'
    text = text.replace(rec_anchor, RECORD_FN + rec_anchor, 1)
    endpoint_anchor = "    @app.post('/api/network-analytics/pageview', include_in_schema=False)\n"
    text = text.replace(endpoint_anchor, ENDPOINT + '\n' + endpoint_anchor, 1)
    ast.parse(text)
    PY.write_text(text)
...
def apply_js():
    ...
    if "'/api/visitor-analytics/internal-click'" in text:
        return
    marker = "  document.addEventListener('prerenderingchange', send);\n"
    text = text.replace(marker, marker + JS_ADD, 1)
    JS.write_text(text)
