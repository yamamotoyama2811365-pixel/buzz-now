...
def render_area(database_url, origin, prefecture, city=None, page=1, per_page=40, status='all'):
    ...
    with _connect(database_url) as conn:
        with conn.cursor() as cur:
            ...
            stats_clauses = ["COALESCE(status,'') <> 'excluded' AND (COALESCE(NULLIF(address,''),'')<>'' OR COALESCE(confidence,0)>=94)","prefecture=%s"]
            ...
            area_clauses = ["COALESCE(status,'') <> 'excluded' AND (COALESCE(NULLIF(address,''),'')<>'' OR COALESCE(confidence,0)>=94)","prefecture=%s"]
...
def render_category(database_url, origin, category, page=1, per_page=40, status='all'):
    ...
    title = f"{category}の開店閉店情報｜全国の新店・閉店一覧｜{SITE_NAME}"
    ...
    if query_parts:
        canonical_path += "?" + "&".join(query_parts)
    desc = f"全国の「{category}」に関する開店閉店情報。新規オープン、開店予定、閉店予定、閉店店舗をエリア別に確認できます。"
...
def render_store(database_url, origin, store_id):
    with _connect(database_url) as conn:
        with conn.cursor() as cur:
            cur.execute(STORE_SELECT + """
                WHERE id=%s AND COALESCE(status,'') <> 'excluded' AND (COALESCE(NULLIF(address,''),'')<>'' OR COALESCE(confidence,0)>=94)
            """,(store_id,))
            r=cur.fetchone()
            if not r:
                return None
            ...
            cur.execute("""
                ...
                FROM stores
                WHERE id<>%s
                  AND COALESCE(status,'') <> 'excluded' AND (COALESCE(NULLIF(address,''),'')<>'' OR COALESCE(confidence,0)>=94)
