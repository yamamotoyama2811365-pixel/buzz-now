/* i-mobile: four inline placements plus one dismissible bottom banner. */
(function () {
  'use strict';
  ...
  function mount() {
    ...
    // Choose content boundaries before adding ads so ads cannot become anchors.
    ...
    if (candidates.length < 2) candidates = Array.from(main.children).filter(function (node) {
    ...
    var targets = [0.34, 0.68].map(function (ratio) {
      var desired = mainBox.top + mainBox.height * ratio;
      return candidates.reduce(function (best, node) {
        return !best || Math.abs(node.getBoundingClientRect().top - desired) < Math.abs(best.getBoundingClientRect().top - desired) ? node : best;
      }, null);
    });
    if (targets[0] && targets[0] === targets[1] && candidates.length > 1) {
      targets = [candidates[Math.floor((candidates.length - 1) / 3)], candidates[Math.max(1, Math.floor((candidates.length - 1) * 2 / 3))]];
    }
    targets = targets.map(function (node) {
      while (node && node.parentElement && node.parentElement !== main && node.parentElement.getBoundingClientRect().width < 300) node = node.parentElement;
      return node;
    });
    ...
    ['top','mid1','mid2','bottom'].forEach(function (position) {
      ...
      label.className = 'sow-imobile-label'; label.textContent = '広告';
      aside.appendChild(label); nodes.push(aside);
      if (position === 'top') {
        var header = document.querySelector('header');
        if (header) header.after(aside); else main.prepend(aside);
      } else if (position === 'bottom') {
        var footer = document.querySelector('footer');
        if (footer) footer.before(aside); else main.appendChild(aside);
      } else {
        var anchor = targets[position === 'mid1' ? 0 : 1];
        if (anchor) anchor.before(aside); else main.appendChild(aside);
      }
    ...
    if (!dismissed) {
      ...
      var head = document.createElement('div'); head.className = 'sow-imobile-overlay-head';
      var label = document.createElement('span'); label.textContent = '広告'; label.style.fontSize = '11px';
      var close = document.createElement('button'); close.type = 'button'; close.className = 'sow-imobile-close';
      close.textContent = '×'; close.setAttribute('aria-label', '追従広告を閉じる');
      close.addEventListener('click', function () { dismissed = true; overlay.remove(); document.body.style.paddingBottom = previousPadding; });
      head.append(label, close); overlay.appendChild(head); document.body.appendChild(overlay); nodes.push(overlay);
    ...
    if (!document.querySelector('script[src^="https://imp-adedge.i-mobile.co.jp/script/v1/spot.js"]')) {
      ...
      loader.src = 'https://imp-adedge.i-mobile.co.jp/script/v1/spot.js?20220104'; document.head.appendChild(loader);
    }
    return cleanup;
  }
  window.SowImobile = {mount:mount,cleanup:function(){if(active)active();},version:'20260917-1'};
  if (!managed) {
